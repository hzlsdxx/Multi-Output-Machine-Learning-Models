...
class TwoStageEasyEnsembleXGBoost:
    ...
    def prepare_data(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Create binary labels (0: zero value, 1: positive value)
    ...
    def create_bayesian_search_space_classifier(self) -> List:
        """Create the Bayesian optimization search space for EasyEnsemble classifier"""
        space = [
            Integer(5, 20, name='easy_ensemble_n_estimators'),  # Number of EasyEnsemble base learners
            Integer(50, 200, name='n_estimators'),  # XGBoost parameter
            Real(0.05, 0.2, name='learning_rate'),
            Integer(3, 8, name='max_depth'),
            Real(0.7, 1.0, name='subsample'),
            Real(0.7, 1.0, name='colsample_bytree'),
            Real(0.001, 0.1, name='reg_alpha'),
            Real(0.001, 0.1, name='reg_lambda'),
        ]
        return space

    def create_bayesian_search_space_regressor(self) -> List:
        """Create the Bayesian optimization search space for the regressor"""
        space = [
    ...
    def bayesian_optimize_classifier(self, X: np.ndarray, y_binary: np.ndarray,
                                     n_calls: int = 30, cv_folds: int = 5) -> Dict[str, Any]:
        """Bayesian optimization for EasyEnsemble classifier parameters, using F1 score as the optimization objective"""
        if self.verbose:
            print("Starting Bayesian optimization for EasyEnsemble classifier parameters...")
        ...
        def objective(**params):
            # Separate EasyEnsemble parameters and XGBoost parameters
            ...
            for train_idx, val_idx in kfold.split(X):
                ...
                # Create EasyEnsemble classifier for each target variable
                fold_f1_scores = []
                for i in range(y_binary.shape[1]):
                    easy_ensemble = EasyEnsembleClassifier(
                        estimator=xgb.XGBClassifier(**current_params),
            ...
            if self.verbose and len(classifier_history) % 10 == 0:
                print(f"EasyEnsemble iteration {len(classifier_history)}, F1 score: {mean_f1:.4f}")

            return -mean_f1  # Negative because we want to maximize F1 score
        ...
        # Separate EasyEnsemble parameters
        ...
        if self.verbose:
            print(f"\nBayesian optimization for EasyEnsemble classifier completed!")
            print(f"Optimization time: {optimization_time:.2f} seconds")
            print(f"Best F1 score: {-result.fun:.4f}")
            print(f"Best number of EasyEnsemble base learners: {self.easy_ensemble_n_estimators}")
    ...
    def bayesian_optimize_regressor(self, X: np.ndarray, y_continuous: np.ndarray, y_binary: np.ndarray,
                                    n_calls: int = 50, cv_folds: int = 5) -> Dict[str, Any]:
        """Bayesian optimization for regressor parameters"""
        if self.verbose:
            print("Starting Bayesian optimization for regressor parameters...")
            print(f"Number of optimization iterations: {n_calls}, Cross-validation folds: {cv_folds}")
        ...
        def objective(**params):
            ...
            # Train regressor using only positive samples
            positive_mask = np.any(y_binary > 0, axis=1)
            X_positive = X[positive_mask]
            y_positive = y_continuous[positive_mask]

            if len(X_positive) == 0:
                return 1000  # If there are no positive samples, return a large loss
            ...
            for train_idx, val_idx in kfold_positive.split(X_positive):
                ...
                for i in range(y_positive.shape[1]):
                    if len(np.unique(y_val_fold[:, i])) > 1:  # Avoid variance = 0 case
                        r2 = r2_score(y_val_fold[:, i], y_pred_fold[:, i])
                        fold_r2_scores.append(r2)

                if fold_r2_scores:  # Add only if there are valid R² scores
            ...
            if self.verbose and len(regressor_history) % 10 == 0:
        ...
        if self.verbose:
            print(f"\nBayesian optimization for the regressor completed!")
            print(f"Optimization time: {optimization_time:.2f} seconds")
        ...
        def fit(self, X: np.ndarray, y_continuous: np.ndarray, y_binary: np.ndarray):
            """Train the two-stage EasyEnsemble model"""
            if self.verbose:
                gpu_status = "GPU" if self.use_gpu else "CPU"
                print(f"Starting training of the two-stage EasyEnsemble model... (using {gpu_status})")
            ...
            # Create an EasyEnsemble classifier for each target variable
            self.classifier = {}
            ...
            # Train the regressor (using only positive samples)
            if self.verbose:
        ...
        def predict(self, X: np.ndarray) -> np.ndarray:
            ...
            if self.classifier is None or self.regressor is None:
                raise ValueError("Model not trained, please call the fit method first")

            # Get EasyEnsemble classification probabilities
            probabilities = np.zeros((X.shape[0], len(self.target_names)))
            for i, target in enumerate(self.target_names):
                prob = self.classifier[target].predict_proba(X)
                probabilities[:, i] = prob[:, 1]  # Probability of positive class

            # Regression prediction
            reg_pred = self.regressor.predict(X)
            reg_pred = np.maximum(reg_pred, 0.0)  # Ensure non-negative

            # Soft combination: probabilities × regression predictions
            final_pred = probabilities * reg_pred

            return final_pred

        def predict_detailed(self, X: np.ndarray) -> Dict[str, np.ndarray]:
            """Detailed prediction, returning classification probabilities and regression predictions"""
            if self.classifier is None or self.regressor is None:
                raise ValueError("Model not trained, please call the fit method first")
        ...
        def cross_validate(self, X: np.ndarray, y_continuous: np.ndarray, y_binary: np.ndarray,
            ...
            for fold, (train_idx, val_idx) in enumerate(kfold.split(X)):
                ...
                if np.sum(positive_mask) > 0:
                    ...
                    continue  # Skip folds with no positive samples
        ...
        def get_feature_importance(self) -> Dict[str, pd.DataFrame]:
            ...
            # EasyEnsemble classifier feature importance
            classifier_importance = pd.DataFrame()
            for i, target in enumerate(self.target_names):
                # EasyEnsemble feature importance needs to be extracted from each base learner
                easy_ensemble = self.classifier[target]

                # Calculate the average feature importance from all base learners
                all_importances = []
    ...
    def print_comprehensive_results(self, cv_results: dict):
        ...
        print("\n" + "=" * 80)
        print("Two-Stage EasyEnsemble-XGBoost Multi-Output Regression Model – Summary")
        print("=" * 80)

        # Classifier performance
        print(f"\nEasyEnsemble Classifier Performance (Base estimators: {self.easy_ensemble_n_estimators}):")
        print(f"{'Target':<15} {'Acc':<8} {'Prec':<8} {'Recall':<8} {'F1':<8}")
        print("-" * 60)
        for target in self.target_names:
        ...
        print(f"{'Target':<15} {'R²_mean':<8} {'R²_std':<8} {'RMSE_mean':<10} {'MAE_mean':<8}")
        print("-" * 55)
        for target in self.target_names:
            results = cv_results[target]
        ...
        # Overall metrics
        print(f"\nOverall Metrics:")
        print(f"  Avg Classification Accuracy: {cv_results['overall']['accuracy_mean']:.4f}")
        print(f"  Avg F1 Score: {cv_results['overall']['f1_mean']:.4f}")
        print(f"  Avg R² Score: {cv_results['overall']['r2_mean']:.4f}")
        print(f"  EasyEnsemble Base Estimators: {cv_results['overall']['easy_ensemble_n_estimators']}")
        print(f"  Total CV Time: {cv_results['overall']['cv_time']:.2f} sec")

def main_easy_ensemble_optimization():
    try:
        df = pd.read_csv('PA_1km_drop_AF_new.csv')
        print(f"Data loaded successfully, shape: {df.shape}")
    except FileNotFoundError:
        print("Please provide the correct CSV file path")
        return
    ...
    print(f"Data preparation done, features: {X.shape[1]}, samples: {X.shape[0]}")
    ...
    # Bayesian optimization — classifier
    print("\n=== Bayesian Optimization: EasyEnsemble Classifier (F1) ===")
    classifier_optimization = model.bayesian_optimize_classifier(
        X, y_binary, n_calls=30, cv_folds=5
    )

    # Bayesian optimization — regressor
    print("\n=== Bayesian Optimization: Regressor ===")
    ...
    # Save model
    model.save_model("final_AF_two_stage_easy_ensemble.pkl")

    print("\n" + "=" * 80)
    print("Two-Stage EasyEnsemble Model Summary")
    print("=" * 80)
    print(f"Best Classifier F1: {classifier_optimization['best_f1_score']:.4f}")
    print(f"Best Regressor R²: {regressor_optimization['best_r2_score']:.4f}")
    print(f"Final Avg F1: {cv_results['overall']['f1_mean']:.4f}")
    print(f"Final Avg R²: {cv_results['overall']['r2_mean']:.4f}")
    print(f"Best EasyEnsemble Base Estimators: {classifier_optimization['best_easy_ensemble_n_estimators']}")
    print(f"Classifier Optimization Time: {classifier_optimization['optimization_time']:.2f} sec")
    print(f"Regressor Optimization Time: {regressor_optimization['optimization_time']:.2f} sec")

    # Best parameters
    print(f"\nBest EasyEnsemble Classifier Parameters:")
    print(f"  Base estimators: {classifier_optimization['best_easy_ensemble_n_estimators']}")
...
def analyze_easy_ensemble_predictions(model, X, y_continuous, sample_size=100):
    """
    Analyze the predictions of EasyEnsemble, especially focusing on the improvements in handling imbalanced data
    """
    print("\n" + "=" * 80)
    print("EasyEnsemble Prediction Analysis")
    print("=" * 80)
    ...
    print(f"Sample size analyzed: {len(X_sample)}")
    print(f"EasyEnsemble base learners: {model.easy_ensemble_n_estimators}")

    for i, target in enumerate(model.target_names):
        print(f"\n{target} Analysis:")

        # Identify historical zero-value samples
        zero_mask = y_sample[:, i] < 1e-6
        zero_count = np.sum(zero_mask)

        if zero_count > 0:
            # Prediction of zero-value samples
            zero_probs = probabilities[zero_mask, i]
            zero_reg_pred = regression_pred[zero_mask, i]
            zero_final_pred = final_pred[zero_mask, i]

            print(f"  Historical zero-value sample count: {zero_count}")
            print(f"  Zero-value samples average probability: {np.mean(zero_probs):.4f} ± {np.std(zero_probs):.4f}")
            print(f"  Zero-value samples median probability: {np.median(zero_probs):.4f}")
            print(f"  Zero-value samples regression prediction mean: {np.mean(zero_reg_pred):.1f}")
            print(f"  Zero-value samples final prediction mean: {np.mean(zero_final_pred):.1f}")
            print(f"  Zero-value samples max final prediction: {np.max(zero_final_pred):.1f}")

            # Checking extreme jumps
            extreme_jumps = zero_final_pred > 50000  # Greater than 50,000 square meters
            extreme_count = np.sum(extreme_jumps)
            print(f"  Extreme jumps (>50,000) sample count: {extreme_count} ({extreme_count / zero_count * 100:.1f}%)")
        ...
        # Prediction of positive-value samples
        ...
        if positive_count > 0:
            ...
            print(f"  Historical positive sample count: {positive_count}")
            print(f"  Positive samples average probability: {np.mean(pos_probs):.4f} ± {np.std(pos_probs):.4f}")
            print(f"  Positive samples median probability: {np.median(pos_probs):.4f}")
            print(f"  Positive samples R²: {r2_score(pos_true, pos_final_pred):.4f}")


if __name__ == "__main__":
    print("=" * 80)
    print("Two-Stage EasyEnsemble-XGBoost Multi-Output Regression Model")
    print("Configuration: EasyEnsemble for handling imbalanced data, optimized for F1 score")
    print("Dependencies:")
    ...
    try:
        ...
        model, classifier_opt, regressor_opt, cv_results = main_easy_ensemble_optimization()

        # Optional: Run prediction analysis
        print("\nDo you want to run the EasyEnsemble prediction result analysis? (y/n, default y): ", end="")
        analysis_choice = input().strip().lower()

        if analysis_choice != 'n':
            # Reload data for analysis
            df = pd.read_csv('PA_1km_drop_AF.csv')
            X, y_continuous, y_binary = model.prepare_data(df)
            analyze_easy_ensemble_predictions(model, X, y_continuous, sample_size=200)

    except KeyboardInterrupt:
        print("\nUser interrupted the run")
    except Exception as e:
        print(f"Run failed: {e}")
        print("Please check if the dependencies are correctly installed")
